...
export function formatMessage(name, age, profession) {
  return `Hello ${name}! You are ${age} years old and work as a ${profession}.
  Welcome to our platform!`;
}
...
export function highlight(strings, ...values) {
  const formattedString = strings.reduce((acc, curr, i) => {
    ...
    return (acc += value ? `${curr}<strong>${value}</strong>` : curr);
  }, '');
...
export function createHTML(user, input) {
  ...
  return `<div>User: ${escapedUser}, Input: ${escapedInput}</div>`;
}
...
export function destructureUser(user) {
  const { name, age = 18, email = 'N/A' } = user;
